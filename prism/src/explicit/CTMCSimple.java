...
public class CTMCSimple extends DTMCSimple implements CTMC
{
	...
	/**
	* Build (a new) discretised DTMCDeadlineSimple for this CTMC.
	* 
	* @param tau Step duration
	*/
	public DTMCDeadlineSimple buildDiscretisedDTMC(double tau)
	{
		DTMCDeadlineSimple dtmc;
		Distribution distrNew;
		int i;
		double sum, d;
		dtmc = new DTMCDeadlineSimple(numStates);
		for (int in : getInitialStates()) {
			dtmc.addInitialState(in);
		}
		for (i = 0; i < numStates; i++) {
			...
			distrNew.add(i, d);
			dtmc.setTransitions(i, distrNew);
		
		}
		return new DTMCDeadlineSimple(dtmc, deadlineTrans);
	}
