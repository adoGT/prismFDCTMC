...
public class DTMCSimple extends DTMCExplicit implements ModelSimple
{
	...
	protected List<Integer> deadlineTrans = null;
	protected Set<Integer> initialStatesToRemove = null;
	...
	/**
	 * Set the transitions (a distribution) for state s.
	 */	
	public void setTransitions(int s, Distribution distribution)
	{
		trans.add(s, distribution);
	}
	...
	public List<Integer> getDeadlineTrans()
	{
		return deadlineTrans;
	}
	
	public void setInitialStatesToRemove(Set<Integer> states)
	{
		initialStatesToRemove = states;
	}
	
	public Set<Integer> getInitialStatesToRemove()
	{
		return initialStatesToRemove;
	}
   
	
	/**
	 * Do a vector matrix multiplication for
	 * the DTMC's transition probability matrix P and the vector 
	 * {@code distr} passed in as Distribution object.
	 * @param distr to be multiply by
	 * @return distribution resulting from multiplication
	 */
    public Distribution multiplyDistribution(Distribution distr) {
        Distribution newDistr= new Distribution();
        for(int i=0; i<numStates; ++i){
            double val = 0;
            for(Map.Entry<Integer,Double> pos : distr) {
                val += pos.getValue()*getTransitions(pos.getKey()).get(i);
            }
            if(val!=0)
                newDistr.add(i, val);
        }
