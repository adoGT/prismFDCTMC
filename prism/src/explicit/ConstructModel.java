...
public class ConstructModel
{
	...
	public Model constructModel(ModulesFile modulesFile, boolean justReach, boolean buildSparse, boolean distinguishActions) throws PrismException
	{
		...
		if (!justReach) {
			...
			case CTMC:
				...
				break;
			case FDCTMC:
				modelSimple = fdctmc = new FDCTMCSimple();
				break;
			case MDP:
				...
				for (j = 0; j < nt; j++) {
					...
					if (!justReach) {
						...
						case CTMC:
							...
							break;
						case FDCTMC:
							int module, indexOfFD;
							module = engine.getTransitionModuleOrActionIndex(i);
							if(module <0) {
								module = -module -1;
								indexOfFD = engine.getTransitionFDIndex(i);
							} else {
								String synchLabel = modulesFile.getSynch(module-1);
								module = modulesFile.getModuleIndexFromFDSynch(synchLabel);
								indexOfFD = modulesFile.getFDIndexFromSynch(synchLabel);
								fdctmc.addSynchLabel(indexOfFD,src, dest, synchLabel);
							}
							String labelOfFD = indexOfFD==-1? "" :modulesFile.getConstantValues().getName(indexOfFD);
							fdctmc.addToProbability(src, dest, engine.getTransitionProbability(i, j), engine.getFDelay(module,  indexOfFD), module, indexOfFD, labelOfFD);
							break;
						case MDP:
		...
		if (sort) {
			...
			statesList = states.toArrayList();
                ...
                /*
                //if it is FDCTMC, find all fix-delays
                if(modelType == ModelType.FDCTMC) {
                    constructFDCTMC(modulesFile, fdctmc);
                }*/
		...
		if (!justReach) {
			...
			case CTMC:
				...
				break;
			case FDCTMC:
				model = sort ? new FDCTMCSimple(fdctmc, permut) : (FDCTMCSimple) fdctmc;
				break;
			case MDP:
				if (buildSparse) {
       ...
       /* 
        private void constructFDCTMC(ModulesFile modulesFile, FDCTMCSimple fdctmc) throws PrismException{
            if(modulesFile.getNumModules()==1) {
                for(int i=0; i<modulesFile.getModule(0).getNumFDelays(); ++i) {
                    FDEvent fdEvent = new FDEvent(fdctmc.getNumStates());
                    fdEvent.setDelay(modulesFile.getModule(0).getFDelayValueOf(modulesFile.getModule(0).getFDelayName(i)));
                    fdctmc.addFDEvent(fdEvent);
                }
                
                for(Command command : modulesFile.getModule(0).getCommands()) {
                    if(command.getFDelay()!="") {
                        FDEvent fdEvent = fdctmc.getFDEvent(modulesFile.getModule(0).getFDelayIndex(command.getFDelay()));
                    }
                }
            }
        }*/
	...
	public static void main(String[] args)
	{
		try {
			...
			model.exportToPrismExplicitTra(mainLog);
