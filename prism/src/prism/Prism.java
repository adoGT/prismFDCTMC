...
public class Prism implements PrismSettingsListener
{
		...
		try {
			...
			case PRISM_MODEL:
				if (!getExplicit()) {
					...
					if(deadlineTrans != null) //TODO check whether it is necessary to set buildSparse to false
					{
						currentModelExpl = constructModel.constructModel(currentModulesFile, false, false);
						//extending the initial states by those used by deadlines 
						//(because some heuristics could delete these states, if they are unreachable by normal transitions)
						//TODO check whether this is good place to do so
						if(currentModelExpl instanceof DTMCSimple)
						{
							DTMCSimple deadlineModel = (DTMCSimple) currentModelExpl;
							Set<Integer> initialStatesToRemove = new HashSet<Integer>();
							for(int i=0;i<deadlineTrans.size();i++)
							{
								if(deadlineModel.isInitialState(deadlineTrans.get(i)))
								{
									deadlineModel.addInitialState(deadlineTrans.get(i));
									initialStatesToRemove.add(i);
								}
							}
							deadlineModel.setInitialStatesToRemove(initialStatesToRemove);
						}
					}
					else
					{
	...
	public Result modelCheck(PropertiesFile propertiesFile, Property prop) throws PrismException, PrismLangException
	{
		...
		if((currentModelExpl instanceof DTMCSimple) && (deadlineTrans != null))
		{
			((DTMCSimple)currentModelExpl).setDeadline(deadline);
			((DTMCSimple)currentModelExpl).setDeadlineTrans(deadlineTrans);
	...
	public void doTransient(double time, int exportType, File fileOut, File fileIn) throws PrismException
	{
		...
		PrismLog tmpLog;
                ...
                System.out.println("MODEL TYPE: " + currentModelType);
                
		if (!(currentModelType == ModelType.CTMC || currentModelType == ModelType.DTMC || currentModelType == ModelType.FDCTMC))
			throw new PrismException("Transient probabilities only computed for DTMCs/CTMCs/FDCTMCs");
		...
		if (!getExplicit()) {
			...
			} else if (currentModelType == ModelType.FDCTMC) {
				FDCTMCModelChecker mcFDCTMC = new FDCTMCModelChecker();
				mcFDCTMC.setLog(mainLog);
				mcFDCTMC.setSettings(settings);
				probsExpl = mcFDCTMC.doTransient((FDCTMCSimple) currentModelExpl, time, fileIn);
			} else {
	...
	public void setDeadlineTrans(List<Integer> deadlineTrans)
	{
		this.deadlineTrans = deadlineTrans;
	}
